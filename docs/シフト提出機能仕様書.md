...
# 備考: 以下は将来的な拡張案（未実装）
# resources :monthly_summaries, only: [:index, :show]
# resources :weekly_shifts, only: [:show, :edit, :update]
```

#### 2.3 参考：将来的なコントローラー実装例（未実装）

以下は週中心設計を採用する場合の実装例です。現在は`ShiftRequestsController`で実装されています。

```ruby
# monthly_summaries_controller.rb（参考実装例 - 未実装）
...
# weekly_shifts_controller.rb（参考実装例 - 未実装）
...
#
# WeeklyShift テーブル:
# 10月提出分: week_id: 44, submission_year: 2025, submission_month: 10, status: 'submitted'
...
#### 2.4 UI表示での前月データ継承
- **前月データ**: `status: :submitted` の WeeklyShift を参照
- **表示形式**: グレーアウト + 編集不可マーカー
- **計算対象**: 週制限チェックには含める
- **データ更新**: 過去月データは変更不可、当月以降のデータは編集可能
