...
2-1. **ユーザー管理機能実装** - 3SP
   - Users モデル・テーブル作成
   - 承認待ち状態管理（AASM: pending/active/inactive）
   - 新規ユーザー承認待ち画面作成
   - **UI統一**：詳細・編集画面の統一（承認待ち/アクティブ/削除済み全て同じUI構成）
...
2-3. **権限管理システム実装** - 3SP ✅
   - Roles, UserRoles モデル作成（単一ロール制約）
   - Pundit gem権限制御
   - 管理者による新規ユーザー承認・編集機能
   - **契約期間管理**：
     - 契約開始日（created_at）・契約終了日の管理
     - アルバイトユーザーは契約終了日必須（デフォルト6ヶ月）
     - 管理者による承認時・編集時の設定機能
   - **バリデーション強化**：
     - 編集画面：姓名・部署・権限は必須
     - アルバイト選択時：契約終了日必須
