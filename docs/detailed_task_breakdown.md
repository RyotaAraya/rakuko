# 学生アルバイト勤怠管理システム「楽工」詳細タスク分解

## Phase 1: 基盤構築

### 1-1. Rails新規プロジェクト作成 - 2SP

**概要**: 学生アルバイト勤怠管理システムのRailsプロジェクトを新規作成し、必要なgemを導入

**詳細タスク**:
- [ ] `rails new rakuko --database=postgresql` でプロジェクト作成
- [ ] 基本gem導入
  - [ ] devise gem（ユーザー認証）
  - [ ] omniauth-google-oauth2 gem（Google OAuth）
  - [ ] pundit gem（権限管理）
  - [ ] aasm gem（状態管理）
  - [ ] sidekiq gem（バックグラウンドジョブ）
- [ ] Gemfile設定・初期セットアップ
  - [ ] bundler設定
  - [ ] 開発・テスト環境gem追加
  - [ ] 初期コミット・GitHubプッシュ

**完了条件**:
- Railsアプリケーションが正常に起動する
- 全ての基本gemが正しく導入されている
- GitHubリポジトリに初期コードがプッシュされている

---

### 1-2. Vue.js + Docker環境構築 - 2SP

**概要**: Vue.js 3 + TypeScriptの部分組み込み設定とDocker開発環境を構築

**詳細タスク**:
- [ ] Vue.js 3 + TypeScript部分組み込み設定
  - [ ] vite_rails gem導入
  - [ ] package.json設定
  - [ ] TypeScript設定ファイル作成
  - [ ] Vite設定ファイル作成
- [ ] Docker環境構築
  - [ ] Dockerfile作成（Ruby 3.2 + Node.js環境）
  - [ ] docker-compose.yml作成（PostgreSQL、Redis含む）
  - [ ] 開発環境用の環境変数設定
  - [ ] Docker内でのVite + Rails連携確認

**完了条件**:
- Dockerコンテナ内でRailsアプリケーションが正常に起動する
- Vue.js開発環境が機能している
- ViteによるHMRが動作している

---

### 1-3. トップページ作成 - 2SP

**概要**: アプリケーションのランディングページを作成し、基本レイアウトとVue.jsの動作を確認

**詳細タスク**:
- [ ] ランディングページ作成
  - [ ] HomeController#index作成
  - [ ] ルート設定（root to: 'home#index'）
  - [ ] 基本的なトップページビュー作成
- [ ] Vue.js部分組み込み
  - [ ] Vue.jsコンポーネント作成（例: WelcomeMessage）
  - [ ] Rails ViewとVue.jsの連携確認
  - [ ] TypeScriptでの基本的な連携確認
- [ ] 基本レイアウト・デザイン
  - [ ] application.html.erb レイアウト作成
  - [ ] ナビゲーションバー基本構造
  - [ ] 基本CSS設定
  - [ ] フラッシュメッセージ表示機能
- [ ] 初期ルーティング設定
  - [ ] routes.rb基本設定

**完了条件**:
- トップページが正常に表示される
- Vue.jsコンポーネントが動作している
- 基本的なナビゲーションが機能している
- 基本的なレイアウトが適用されている

---

### 1-4. Render本番環境構築 - 2SP

**概要**: Render本番環境設定とPostgreSQLデータベース接続

**詳細タスク**:
- [ ] Render本番環境設定
  - [ ] Renderアカウント作成・設定
  - [ ] GitHubリポジトリ連携
  - [ ] 本番環境ビルド・デプロイ設定
- [ ] PostgreSQL接続設定
  - [ ] Render PostgreSQLサービス作成（本番）
  - [ ] database.yml本番環境設定
  - [ ] データベース接続テスト
- [ ] 環境変数設定・管理
  - [ ] SECRET_KEY_BASE設定（本番）
  - [ ] DATABASE_URL設定（本番）
  - [ ] Google OAuth設定（本番）
  - [ ] 環境変数管理シート作成

**完了条件**:
- Render本番環境へのデプロイが成功している
- 本番環境でアプリケーションが正常に動作している
- PostgreSQLデータベースに接続できている
- 環境設定が適切に分離されている

---

### 1-5. RenderSTG環境構築 - 1SP

**概要**: Renderステージング環境設定とSTG用PostgreSQLデータベース接続

**詳細タスク**:
- [ ] RenderSTG環境設定
  - [ ] STG環境ビルド・デプロイ設定
  - [ ] STGブランチ連携設定
- [ ] STG用PostgreSQL接続設定
  - [ ] Render PostgreSQLサービス作成（STG）
  - [ ] database.ymlSTG環境設定
  - [ ] STGデータベース接続テスト
- [ ] STG環境変数設定
  - [ ] SECRET_KEY_BASE設定（STG）
  - [ ] DATABASE_URL設定（STG）
  - [ ] Google OAuth設定（STG別）

**完了条件**:
- RenderSTG環境へのデプロイが成功している
- STG環境でアプリケーションが正常に動作している
- STG用PostgreSQLデータベースに接続できている

---

### 1-6. Google OAuth認証機能実装 - 3SP

**概要**: Devise gemとGoogle OAuthを使用したユーザー認証システムの実装

**詳細タスク**:
- [ ] Devise gem設定
  - [ ] `rails generate devise:install`実行
  - [ ] devise設定ファイル調整
  - [ ] Userモデル生成・マイグレーション
- [ ] Google OAuth設定
  - [ ] Google Cloud Console設定
  - [ ] OAuth 2.0 クライアントID作成
  - [ ] omniauth-google-oauth2 gem設定
- [ ] ログイン・ログアウト機能
  - [ ] devise viewsカスタマイズ
  - [ ] Googleログインボタン実装
  - [ ] ログアウト機能実装
  - [ ] 認証後のリダイレクト設定

**完了条件**:
- Googleアカウントでのログインが機能している
- ログアウト機能が正常に動作している
- 未認証ユーザーは適切にリダイレクトされる
- ユーザー情報が正しくデータベースに保存されている

---

## Phase 2: ユーザー・権限管理

### 2-1. ユーザー管理機能実装 - 3SP

**概要**: ユーザー情報管理と新規ユーザーの承認待ち機能を実装

**詳細タスク**:
- [ ] Users モデル・テーブル拡張
  - [ ] マイグレーション作成・実行
  - [ ] バリデーション設定
  - [ ] 関連付け設定（belongs_to :department等）
- [ ] 承認待ち状態管理
  - [ ] ステータス管理（pending, approved, rejected）
  - [ ] AASM gem使用の状態遷移定義
  - [ ] 承認・拒否メソッド実装
- [ ] 新規ユーザー承認待ち画面作成
  - [ ] 承認待ち専用コントローラー・ビュー作成
  - [ ] 承認待ち中のメッセージ表示
  - [ ] 連絡先案内・手順説明表示

**完了条件**:
- 新規ユーザーは自動的に承認待ち状態になる
- 承認待ち画面が適切に表示される
- ユーザー状態の変更が正常に動作する
- データベース設計が要件を満たしている

---

### 2-2. 部署管理機能実装 - 2SP

**概要**: 部署の作成・編集・削除機能と部署担当者管理

**詳細タスク**:
- [ ] Departments モデル・テーブル作成
  - [ ] テーブル設計（name, description, manager_id等）
  - [ ] マイグレーション作成・実行
  - [ ] バリデーション設定
  - [ ] 関連付け設定（has_many :users等）
- [ ] 部署CRUD機能
  - [ ] DepartmentsController作成
  - [ ] 部署一覧・詳細・新規作成・編集・削除機能
  - [ ] 部署管理画面のビュー作成
  - [ ] 部署担当者割り当て機能
  - [ ] 権限チェック（管理者のみアクセス可能）

**完了条件**:
- 部署の作成・編集・削除が正常に動作する
- 部署担当者の設定ができる
- ユーザーと部署の関連付けが機能している
- 管理者権限でのみアクセス可能

---

### 2-3. 権限管理システム実装 - 3SP

**概要**: 4つの権限レベル（アルバイト/部署担当者/労務担当者/システム管理者）の実装

**詳細タスク**:
- [ ] Roles, UserRoles モデル作成
  - [ ] Rolesテーブル設計・作成
  - [ ] UserRolesテーブル設計・作成（多対多関係）
  - [ ] 初期ロールデータ作成（4つの権限レベル）
  - [ ] モデル関連付け設定
- [ ] Pundit gem権限制御
  - [ ] ApplicationPolicy基底クラス作成
  - [ ] 各機能別Policy作成
  - [ ] 権限チェックメソッド実装
  - [ ] ビューでの権限別表示制御
- [ ] 管理者による新規ユーザー承認機能
  - [ ] ユーザー承認画面作成
  - [ ] 権限割り当て機能
  - [ ] 一括承認・拒否機能
  - [ ] 承認履歴管理

**完了条件**:
- 4つの権限レベルが正しく設定されている
- Punditによる権限制御が機能している
- 管理者がユーザーを承認・権限設定できる
- 権限に応じた画面表示制御が動作している

---

## Phase 3: コア機能（勤怠・シフト）

### 3-1. シフトモデル作成 - 2SP

**概要**: 月次シフト管理と日次スケジュールのデータモデル作成

**詳細タスク**:
- [ ] Shifts, ShiftSchedules モデル作成
  - [ ] Shiftsテーブル設計（月次シフト管理）
  - [ ] ShiftSchedulesテーブル設計（日次スケジュール）
  - [ ] 掛け持ちバイト時間フィールド追加
  - [ ] マイグレーション作成・実行
  - [ ] モデル関連付け・バリデーション

**完了条件**:
- ShiftsとShiftSchedulesモデルが正しく作成されている
- 掛け持ちバイト時間のフィールドが含まれている
- データベースマイグレーションが成功している
- モデル関連とバリデーションが機能している

---

### 3-2. シフト入力フォーム実装 - 3SP

**概要**: 月次シフト希望提出フォームと動的ユーザーインターフェースの実装

**詳細タスク**:
- [ ] 月次シフト入力フォーム（Vue.js）
  - [ ] カレンダー形式のシフト入力UI
  - [ ] 日次での勤務時間入力
  - [ ] 掛け持ちバイト時間入力
  - [ ] 入力時の計算表示
- [ ] シフト提出・編集機能
  - [ ] シフトデータの保存機能
  - [ ] 編集モード切り替え
  - [ ] 入力バリデーション

**完了条件**:
- 月次シフト希望が正しく提出できる
- 掛け持ちバイト時間を含む入力が可能
- Vue.jsでの動的UIが正常に動作している
- シフトデータの編集が機能している

---

### 3-3. 労働時間制限チェック機能実装 - 2SP

**概要**: シフト入力時の労働時間制限の事前チェック機能

**詳細タスク**:
- [ ] 週20h/40h制限の事前チェック機能
  - [ ] 週単位での労働時間計算ロジック
  - [ ] 制限違反時のアラート表示
  - [ ] 制限内での最大時間表示
  - [ ] 入力値の動的検証

**完了条件**:
- 労働時間制限のチェックが機能している
- 制限違反時のアラートが適切に表示される
- 入力時のリアルタイムチェックが動作している

---

### 3-4. 実勤怠労働時間集計機能実装 - 3SP

**概要**: 勤怠データから週次労働時間を自動計算し、制限違反をチェック

**詳細タスク**:
- [ ] 勤怠データからの週次労働時間自動計算
  - [ ] 週単位での勤怠データ集計ロジック
  - [ ] 弊社勤務時間と掛け持ちバイト時間の分離計算
  - [ ] 休憩時間を除いた実労働時間計算
  - [ ] 週の開始日設定（月曜開始等）
- [ ] 制限違反時のアラート表示機能
  - [ ] 週20時間制限（弊社分）チェック
  - [ ] 週40時間制限（総労働時間）チェック
  - [ ] 違反時のアラートメッセージ生成
  - [ ] ダッシュボードでの警告表示
  - [ ] 管理者への通知機能

**完了条件**:
- 週次労働時間が正確に計算される
- 制限違反が適切に検知される
- アラート表示が正常に機能する
- 管理者が制限違反を確認できる

---

### 3-5. 勤怠登録機能実装 - 3SP

**概要**: 日次の出退勤・休憩時間打刻機能と勤怠状況表示

**詳細タスク**:
- [ ] TimeRecords モデル作成
  - [ ] テーブル設計（出勤時刻、退勤時刻、休憩時間等）
  - [ ] マイグレーション作成・実行
  - [ ] バリデーション設定
  - [ ] 関連付け設定（belongs_to :user等）
- [ ] 出退勤・休憩時間打刻機能（Vue.js）
  - [ ] 打刻ボタンUI作成
  - [ ] 出勤・退勤・休憩開始・休憩終了の状態管理
  - [ ] 打刻履歴の即座反映
- [ ] 6時間以上勤務時の休憩時間自動計算
  - [ ] 勤務時間（6時間以上）チェックロジック
  - [ ] 1時間休憩時間自動計算・適用
  - [ ] 休憩時間を除いた実労働時間計算
- [ ] 勤怠状況一覧表示
  - [ ] 日別・週別・月別勤怠一覧
  - [ ] 労働時間集計表示
  - [ ] 制限時間との比較表示

**完了条件**:
- 出退勤打刻が正常に記録される
- 休憩時間が自動計算される
- 勤怠一覧が正確に表示される
- Vue.jsでの基本的な動作が機能している

---

## Phase 4: 申請・承認ワークフロー

### 4-1. 欠勤申請機能実装 - 2SP

**概要**: 欠勤申請機能の基盤実装

**詳細タスク**:
- [ ] Applications モデル作成
  - [ ] テーブル設計（申請種別、申請日、理由等）
  - [ ] マイグレーション作成・実行
  - [ ] バリデーション設定
- [ ] 欠勤申請フォーム作成
  - [ ] Vue.jsコンポーネント作成
  - [ ] 申請日・理由入力フォーム
  - [ ] バリデーション・エラーハンドリング
- [ ] 欠勤申請一覧表示
  - [ ] 申請一覧ページ作成
  - [ ] 状態別フィルタリング
  - [ ] 申請詳細表示機能

**完了条件**:
- 欠勤申請が正常に提出できる
- 申請一覧が表示される
- Applicationsモデルが正しく動作する

---

### 4-2. 遅刻・早退申請機能実装 - 2SP

**概要**: 遅刻申請と早退申請フォームの実装

**詳細タスク**:
- [ ] 遅刻申請フォーム作成
  - [ ] 遅刻申請専用フォーム
  - [ ] 日時選択機能
  - [ ] 理由入力フィールド
- [ ] 早退申請フォーム作成
  - [ ] 早退申請専用フォーム
  - [ ] 日時選択機能
  - [ ] 理由入力フィールド
- [ ] 申請状況一覧・詳細表示
  - [ ] 申請状況一覧表示
  - [ ] 申請詳細画面
  - [ ] 申請状況（pending, approved, rejected）表示

**完了条件**:
- 遅刻・早退申請が正常に提出できる
- 申請状況が適切に表示される
- 申請フォームのバリデーションが機能している

---

### 4-3. 承認モデル設計・実装 - 3SP

**概要**: 承認システムの基盤となるモデルとAAMS状態管理の実装

**詳細タスク**:
- [ ] Approvals モデル作成（ポリモーフィック）
  - [ ] ポリモーフィック関連付け設計
  - [ ] 承認者種別（department, labor）管理
  - [ ] 承認状態管理（pending, approved, rejected）
  - [ ] マイグレーション作成・実行
- [ ] AASM gem状態管理
  - [ ] 申請状態遷移定義
  - [ ] 状態変更時のコールバック
  - [ ] 状態履歴管理
- [ ] 基本的な承認ロジック実装
  - [ ] 承認・拒否メソッド実装
  - [ ] 承認条件チェック

**完了条件**:
- Approvalsモデルが正しく作成されている
- AASM状態管理が機能している
- 基本的な承認ロジックが動作する

---

### 4-4. 並列承認システム実装 - 2SP

**概要**: 部署担当者と労務担当者による並列承認システムの実装

**詳細タスク**:
- [ ] 並列承認システム
  - [ ] 部署担当者・労務担当者同時承認ロジック
  - [ ] 承認者への通知機能
  - [ ] 最終承認判定ロジック
- [ ] 承認条件ロジック実装
  - [ ] 両方の承認完了チェック
  - [ ] 承認者権限チェック

**完了条件**:
- 並列承認システムが正常に動作する
- 両方の承認完了で最終承認される
- 承認条件ロジックが機能している

---

### 4-5. 承認UI・進捗表示実装 - 3SP

**概要**: 承認状況の可視化と基本的な更新UI

**詳細タスク**:
- [ ] 承認状況可視化・進捗表示（Vue.js）
  - [ ] 承認フロー進捗バー
  - [ ] 承認者別状況表示
- [ ] 基本的な状況更新機能
  - [ ] ページリロード時の状況反映
  - [ ] 承認状況の画面表示更新
- [ ] 承認完了画面表示機能
  - [ ] 申請者への完了メッセージ表示
  - [ ] 管理者ダッシュボードでの完了状況表示

**完了条件**:
- 承認状況が視覚的に分かりやすく表示される
- 承認完了時に適切な画面表示がされる
- 基本的な状況更新機能が動作する

---

### 4-6. 月末締め機能実装 - 2SP

**概要**: 月末の勤怠締め処理機能

**詳細タスク**:
- [ ] MonthEndClosings モデル作成
  - [ ] 月末締めデータ管理テーブル
  - [ ] 締め状況（open, closed）管理
  - [ ] 締め日時記録
  - [ ] マイグレーション作成・実行
- [ ] 締め状況管理機能
  - [ ] 月末締め実行機能
  - [ ] 締め状況表示機能
  - [ ] 締め履歴管理
- [ ] 締め完了チェック機能
  - [ ] 必要なデータの存在チェック
  - [ ] 申請・承認完了チェック
  - [ ] 締め後のデータ保護

**完了条件**:
- 月末締め処理が正常に実行される
- 締め状況が適切に管理される

---

### 4-7. 外部システム連携機能実装 - 1SP

**概要**: 月末締め処理と外部システム（ラクロー・ジョブカン）連携

**詳細タスク**:
- [ ] MonthEndClosings モデル作成
  - [ ] 月末締めデータ管理テーブル
  - [ ] 締め状況（open, closed）管理
  - [ ] 締め日時記録
  - [ ] マイグレーション作成・実行
- [ ] 外部システム連携機能
  - [ ] ラクロー遷移リンク・チェック項目作成
  - [ ] ジョブカン遷移リンク・交通費申請ガイド
  - [ ] チェックリスト完了状況管理
  - [ ] 承認ルート設定リマインダー表示

**完了条件**:
- 月末締め処理が正常に実行される
- チェックリストによる漏れ防止ができている

---

## Phase 5: 管理機能・自動化

### 5-1. ダッシュボード基盤実装 - 2SP

**概要**: ダッシュボードの共通基盤作成

**詳細タスク**:
- [ ] 共通UI基盤作成
  - [ ] ダッシュボード基本レイアウト作成
  - [ ] 共通コンポーネント作成
  - [ ] ナビゲーション統一
- [ ] Pundit gem権限別表示切り替え
  - [ ] DashboardPolicy作成
  - [ ] 権限別アクセス制御
  - [ ] 表示コンテンツの動的切り替え
- [ ] 基本レイアウト作成
  - [ ] ヘッダー・サイドバー作成
  - [ ] メインコンテンツエリア設計
  - [ ] 基本スタイル適用

**完了条件**:
- 共通ダッシュボードレイアウトが完成
- 権限制御が正常に動作する
- 基本的なナビゲーションが機能する

---

### 5-2. ユーザータイプ別ダッシュボード実装 - 4SP

**概要**: 4つのユーザータイプ別ダッシュボードの実装

**詳細タスク**:
- [ ] 4つのユーザータイプ別コンテンツ作成
  - [ ] アルバイト用（勤怠状況、申請状況、シフト確認）
  - [ ] 部署担当者用（部署内承認待ち一覧、アルバイト管理）
  - [ ] 労務担当者用（全社承認待ち、制限チェック結果）
  - [ ] システム管理者用（ユーザー管理、利用統計）
- [ ] Vue.jsで動的更新
  - [ ] 基本的なデータ表示
  - [ ] 権限別データ取得API実装
- [ ] ユーザータイプ別機能実装
  - [ ] 各タイプ専用機能の実装
  - [ ] データフィルタリング
  - [ ] 表示内容の最適化

**完了条件**:
- 権限に応じた適切なダッシュボードが表示される
- 各ユーザータイプの必要情報が見やすく配置されている
- 基本的な動的更新が機能している

---

### 5-3. 新規ユーザー承認・部署管理機能実装 - 3SP

**概要**: 新規ユーザー承認と部署担当者設定の管理機能

**詳細タスク**:
- [ ] 新規ユーザー承認機能
  - [ ] 承認待ちユーザー一覧表示
  - [ ] ユーザー情報確認・承認/拒否
  - [ ] 一括承認・拒否機能
- [ ] 権限設定・部署割り当て機能
  - [ ] 権限選択UI（4つの権限レベル）
  - [ ] 部署割り当て機能
  - [ ] 承認通知機能
- [ ] 部署担当者設定機能
  - [ ] 部署担当者割り当て画面
  - [ ] 担当者変更機能
  - [ ] 承認権限の適切な設定
- [ ] 部署メンバー管理機能
  - [ ] 部署メンバー一覧表示
  - [ ] メンバー追加・削除機能

**完了条件**:
- 新規ユーザーの承認処理が効率的に行える
- 権限設定・部署割り当てが正確に反映される
- 部署担当者の設定・変更が正常に機能する
- 管理機能の権限制御が適切に動作する

---

### 5-4. 日次勤怠サマリバッチ実装 - 2SP

**概要**: 日次での勤怠データ集計自動化

**詳細タスク**:
- [ ] 基本的なSidekiqジョブ作成
  - [ ] 日次サマリジョブクラス作成
  - [ ] Sidekiq基本設定
- [ ] 日次勤怠サマリ作成ロジック
  - [ ] 前日分の打刻データ集計
  - [ ] 勤怠サマリテーブル作成
  - [ ] 集計データの保存処理
- [ ] 手動実行機能
  - [ ] 管理者が手動でバッチ実行できる機能
  - [ ] 基本的なエラーハンドリング

**完了条件**:
- 日次バッチが手動で実行できる
- 勤怠サマリが正確に作成される
- 基本的なエラー処理が機能している

---

### 5-5. 週次労働時間チェックバッチ実装 - 2SP

**概要**: 週次労働時間制限チェック機能の実装（MVP必須バッチ処理）

**詳細タスク**:
- [ ] 週次労働時間制限チェックバッチ
  - [ ] 週単位労働時間計算ロジック
  - [ ] 制限違反検知ロジック（週20h/40h）
  - [ ] 違反時のアラート・警告生成
- [ ] バッチ実行機能
  - [ ] 手動バッチ実行機能（管理者用）
  - [ ] チェック結果のダッシュボード表示
  - [ ] 基本的なエラーハンドリング

**完了条件**:
- 週次労働時間制限違反が正しく検知される
- バッチが手動で実行できる
- チェック結果がダッシュボードに表示される

---

## Phase 6: 品質向上・完成

### 6-1. 最終調整・バグ修正 - 4SP

**概要**: UI/UX改善、総合テスト、パフォーマンス最適化

**詳細タスク**:
- [ ] UI/UX調整・改善
  - [ ] 全画面のデザイン統一
  - [ ] エラーメッセージの分かりやすさ向上
- [ ] 総合テスト・修正
  - [ ] 全機能の結合テスト
  - [ ] 権限別機能テスト
  - [ ] ブラウザ間互換性テスト
  - [ ] パフォーマンステスト
  - [ ] 発見されたバグの修正
- [ ] パフォーマンス最適化
  - [ ] データベースクエリ最適化
  - [ ] Vue.jsコンポーネント最適化
  - [ ] 画像・アセット最適化
  - [ ] キャッシュ戦略実装
- [ ] ユーザビリティ改善
  - [ ] 操作フローの改善
  - [ ] フィードバック・ヘルプ機能追加
  - [ ] エラー処理の改善
  - [ ] ローディング状態の表示改善

**完了条件**:
- 全機能が安定して動作する
- パフォーマンスが要求水準を満たしている
- ユーザビリティが十分に向上している
- 本番運用に耐えうる品質に達している

---

## MVP後の拡張機能

### 通知システム基盤実装

**概要**: メール・Slack通知の基盤システム構築

**詳細タスク**:
- [ ] 通知テーブル設計・作成
  - [ ] Notifications モデル作成
  - [ ] 通知タイプ・送信先管理
  - [ ] 通知履歴・配信状況記録
- [ ] メール通知機能
  - [ ] ActionMailer設定
  - [ ] メールテンプレート作成
  - [ ] 承認・申請完了通知
- [ ] 通知設定管理
  - [ ] ユーザー別通知設定
  - [ ] 通知ON/OFF切り替え
  - [ ] 通知頻度設定

### 月次勤怠集計レポート

**概要**: 月次での勤怠データ集計・レポート機能

**詳細タスク**:
- [ ] 勤怠集計機能
  - [ ] 月次勤怠データ集計ロジック
  - [ ] 部署別・個人別集計
  - [ ] 労働時間・制限違反集計
- [ ] レポート出力機能
  - [ ] PDF/Excel出力機能
  - [ ] グラフ・チャート生成
  - [ ] 管理者向けサマリレポート
- [ ] レポート配信機能
  - [ ] 定期レポート自動配信
  - [ ] メール添付・送信機能
  - [ ] レポート履歴管理

### RSpec（モデルテスト・システムテスト）

**概要**: テストコード整備による品質向上

**詳細タスク**:
- [ ] RSpec環境構築
  - [ ] rspec-rails gem導入
  - [ ] factory_bot, capybara等のテスト用gem
  - [ ] テスト用データベース設定
- [ ] モデルテスト作成
  - [ ] User, Department, Shift等モデルテスト
  - [ ] バリデーション・関連付けテスト
  - [ ] ビジネスロジックテスト
- [ ] システムテスト作成
  - [ ] ログイン・認証フローテスト
  - [ ] 申請・承認ワークフローテスト
  - [ ] UI操作・画面遷移テスト

### CI（GitHubActions）

**概要**: 継続的インテグレーション環境構築

**詳細タスク**:
- [ ] GitHub Actions設定
  - [ ] .github/workflows/ci.yml作成
  - [ ] テスト実行環境構築
  - [ ] PostgreSQL・Redis サービス設定
- [ ] 自動テスト実行
  - [ ] プルリクエスト時の自動テスト
  - [ ] main ブランチマージ時のテスト
  - [ ] テスト結果通知設定
- [ ] デプロイ自動化
  - [ ] STG環境自動デプロイ
  - [ ] 本番デプロイ承認フロー
  - [ ] デプロイ後の動作確認自動化

### Slack通知機能

**概要**: 業務フローに合わせたSlack通知機能

**詳細タスク**:
- [ ] Slack API連携
  - [ ] Slack アプリ作成・設定
  - [ ] Webhook URL設定
  - [ ] 認証・権限設定
- [ ] 通知機能実装
  - [ ] 申請・承認完了通知
  - [ ] 労働時間制限違反アラート
  - [ ] 月末締め完了通知
- [ ] 通知カスタマイズ
  - [ ] チャンネル別通知設定
  - [ ] メンション・通知対象設定
  - [ ] 通知テンプレート管理

---

## 各issueでの説明欄テンプレート

```markdown
## 概要
[機能の概要を簡潔に記載]

## 作業内容
- [ ] [具体的なタスク1]
- [ ] [具体的なタスク2]
- [ ] [具体的なタスク3]

## 完了条件
- [完了の判断基準1]
- [完了の判断基準2]
- [完了の判断基準3]

## 関連ファイル・技術
- [関連するファイルパス]
- [使用する技術・gem]

## 注意事項
- [実装時の注意点があれば記載]
```